...
Чтобы NAT применялся **только к конкретному удалённому IP (например, 10.222.199.31)**,
...
* `10.222.199.31/32 -> next-hop = LAN-IP Windows Server`
...
```bat
route -p add 10.222.199.31 mask 255.255.255.255 <LAN_IP_WIN_SERVER>
```

Проверка (на клиенте):

```bat
tracert 10.222.199.31
```
