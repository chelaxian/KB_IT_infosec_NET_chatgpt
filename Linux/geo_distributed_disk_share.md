...
Вот как оформить `gluster_watchdog.sh` в виде `systemd`-сервиса, который будет запускаться при старте системы и автоматически перезапускаться при сбое.

---

### 🔧 Шаг 1. Скрипт `gluster_watchdog.sh`

Создай файл:

```bash
nano /usr/local/bin/gluster_watchdog.sh
```
...
check_nodes() {
  for ip in "${NODES[@]}"; do
    nc -z -w3 "$ip" $PORT
    if [ $? -ne 0 ]; then
      echo "[$(date)] ERROR: $ip:$PORT недоступен!"
      return 1
    fi
  done
  return 0
...
while true; do
  check_nodes
  if [ $? -ne 0 ]; then
    echo "[$(date)] Перезапуск glusterd..."
    systemctl restart glusterd
    sleep 10
  else
    echo "[$(date)] Все узлы в порядке."
  fi
  sleep $INTERVAL
done
```
...
```bash
chmod +x /usr/local/bin/gluster_watchdog.sh
```
...
```bash
nano /etc/systemd/system/gluster-watchdog.service
```
...
```ini
[Unit]
Description=GlusterD Watchdog Service
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/gluster_watchdog.sh
Restart=always
RestartSec=10
User=root
...
systemctl enable --now gluster-watchdog.service
```
...
```bash
systemctl status gluster-watchdog.service
```

Логи:

```bash
journalctl -u gluster-watchdog.service -f
```
---
