...
2. **Интерактивное меню** выбора: профиль **dense / moe**, **reasoning on/off**, **размер контекста**, плюс «последние настройки» и «быстрый старт».
...
4. **Управление VRAM на RTX 4090**:
   ...
   - LM Studio: **не форсировать GPU offload 1.00**, `--parallel 1`, динамический расчёт offload + fallback по контексту/оффлоаду.
5. **Вспомогательные ярлыки**: остановка LM Studio, выгрузка llama.cpp/прокси, логи, **ngrok** для шаринга локального порта.
6. **claude-mem** (опционально для облачных сценариев; в локальных ярлыках по умолчанию выключен) — с автопочинкой установки.
...
```powershell
git clone https://github.com/<your-fork-or-upstream>/free-claude-code.git "D:\qwen-local-setup\free-claude-code"
```

(Точный URL зависит от того, где вы храните форк/зеркало.)
...
- **`Get-RecommendLmGpuOffload`**: оценка offload **без дефолта 1.00**, с запасом под desktop (dense ~2048 MiB reserve, moe ~1792 MiB), clamp **0.90–0.95**.
- **Загрузка модели**: `--parallel 1 --yes` (снижает `n_parallel` и память).
- **Fallback**: уменьшение контекста и ступени offload (dense: 0.92/0.90/0.88/0.85).
- Перед preload: `unload --all` + пауза (анти-«хвосты» прошлой сессии).
...
- Сохраняет последний выбор в `%USERPROFILE%\.qwen-local-setup\claude-local-menu.<runtime>.last.json`.
...
### 12.2. `scripts/run-claude-local-menu.ps1`
...
function Show-ArrowMenu {
  ...
  if (-not $Options -or $Options.Count -eq 0) {
    throw "Show-ArrowMenu: пустой список опций"
  }
  ...
  while ($true) {
    ...
    Write-Host "Навигация: ↑/↓, Enter — выбрать" -ForegroundColor DarkGray
    Write-Host ""
    for ($i = 0; $i -lt $Options.Count; $i++) {
      $prefix = if ($i -eq $index) { "❯ " } else { "  " }
      $color = if ($i -eq $index) { "Green" } else { "Gray" }
...
if (-not $selected) {
  ...
  $startOptions += [ordered]@{
    id    = "manual"
    label = "Ручной выбор (3 вопроса)"
  }

  $defaultStartIndex = 0
  $startChoice = Show-ArrowMenu -Title "Как запустить сессию?" -Options $startOptions -DefaultIndex $defaultStartIndex
  ...
  if ($startChoice.id -eq "last" -and $last) {
    ...
    $modelOptions = @(
      [ordered]@{ id = "moe"; label = "35B-A3B (MoE) — быстрее, default" },
      [ordered]@{ id = "dense"; label = "27B (Dense) — quality-first" }
    )
    $m = Show-ArrowMenu -Title "1) Какую модель выбрать?" -Options $modelOptions -DefaultIndex 0
    $profile = [string]$m.id

    $reasoningOptions = @(
      [ordered]@{ id = "off"; label = "Обычная (без reasoning) — default" },
      [ordered]@{ id = "on"; label = "С reasoning" }
    )
    $r = Show-ArrowMenu -Title "2) Какой режим ответа?" -Options $reasoningOptions -DefaultIndex 0
    ...
    $c = Show-ArrowMenu -Title "3) Размер контекстного окна?" -Options $ctxOptions -DefaultIndex $defaultCtxIndex
...
### 12.3. `scripts/share-ngrok-local.ps1`
...
### 12.4. `scripts/stop-local-qwen-now.ps1`
...
### 12.5. `scripts/unload-llamacpp-and-proxies.ps1`
...
### 12.6. `scripts/install-llamacpp-win-cuda.ps1` (полный файл из репозитория)
...
### 12.7. `scripts/start-claude-mem.ps1` (с self-repair)
...
### 13.0 Дословный фрагмент первого ответа ассистента (обрыв лимитом платформы)

<details>
<summary>Нажмите, чтобы раскрыть полный текст `run-claude-local-session.ps1` (как в текущей установке)</summary>
...
function Invoke-LmsSafe {
  ...
  if (-not $IgnoreFailure -and $code -ne 0) {
    throw "lms command failed: $($Args -join ' ')"
  }
  return $code
}
...
function Ensure-LMStudioGui {
  ...
  if (-not (Test-LMStudioProcessRunning)) {
    Write-Host "LM Studio: запускаю GUI ($GuiExe)..." -ForegroundColor Cyan
    Start-Process -FilePath $GuiExe -WindowStyle Normal | Out-Null
  } else {
    Write-Host "LM Studio: GUI уже запущен." -ForegroundColor DarkGray
  }
  Write-Host "LM Studio: жду инициализацию (${WarmupSeconds}s)..." -ForegroundColor DarkGray
...
function Try-LoadOnce([string]$LmsExe,[string]$ModelId,[int]$Context,[int]$TimeoutSec,[double]$GpuOffload) {
  ...
  $p = Start-Process -FilePath $LmsExe -ArgumentList $lmsArgs -PassThru -WindowStyle Hidden
...
function Load-ModelBestEffort([string]$LmsExe,[int]$PreferredContext,[double]$GpuOffload) {
  ...
  foreach ($candidate in $candidates) {
    Write-Host "Trying to preload $candidate with ctx=$PreferredContext ..." -ForegroundColor DarkCyan
    ...
    if (Try-LoadOnce -LmsExe $LmsExe -ModelId $candidate -Context $PreferredContext -TimeoutSec 180 -GpuOffload $GpuOffload) {
      Write-Host "Model preloaded: $candidate" -ForegroundColor Green
      return $true
    }
  }
...
function Ensure-ModelLoaded([string]$LmsExe,[string]$ModelId,[int]$Context,[double]$GpuOffload) {
  Write-Host "Ensuring model is loaded with ctx=$Context ..." -ForegroundColor DarkCyan
  ...
  $gpuCandidates = if ($SessionProfile -eq "dense") {
    # Dense profile: keep GPU-first behavior, but never force 1.00 by default.
    @($GpuOffload, 0.92, 0.90, 0.88, 0.85) |
      Where-Object { $_ -ge 0.85 -and $_ -le 0.95 } |
      Select-Object -Unique
  } else {
    # MoE profile: allow modest GPU fallback only after full-GPU attempt.
    @($GpuOffload, 0.95, 0.92, 0.90, 0.87) |
    ...
    foreach ($gpuTry in $gpuCandidates) {
      ...
      if ($code -eq 0 -or $looksSuccessful) {
        if ($ctxTry -ne $Context -or [Math]::Abs($gpuTry - $GpuOffload) -gt 0.0001) {
          Write-Host "Loaded with fallback: ctx=$ctxTry gpu=$gpuArg" -ForegroundColor Yellow
        }
        return
      }
...
function Ensure-LlamaCppServer {
  ...
  for ($i = 0; $i -lt 1800; $i++) {
    ...
    if (Test-OpenAIModelsEndpoint -BaseUrl $openAIBase -TimeoutSec 2) {
      ...
      try {
        $ctxLine = (Get-Content -LiteralPath $logPath -ErrorAction SilentlyContinue | Select-String -Pattern "llama_context: n_ctx\\s*=\\s*(\\d+)" -AllMatches | Select-Object -Last 1)
        if ($ctxLine -and $ctxLine.Matches.Count -gt 0) {
          $nctx = [int]$ctxLine.Matches[0].Groups[1].Value
          if ($nctx -gt 32768) {
            throw "llama.cpp поднялся с n_ctx=$nctx (это сильно режет скорость). Понижаю контекст и перезапускаю."
          }
        }
      } catch {
        # Re-throw to let caller fall back to smaller ctx candidates.
        throw
      }
...
function Get-RecommendLmGpuOffload {
  ...
  $modelPath = Get-PreferredModelPathForProfile -Profile $Profile
  if (-not (Test-Path -LiteralPath $modelPath)) {
    # Keep a desktop-safe fallback if model file size cannot be read.
    return [Math]::Round([Math]::Min(0.95, [Math]::Max(0.90, $Requested)), 2)
  }

  $sizeMiB = [Math]::Ceiling((Get-Item -LiteralPath $modelPath).Length / 1MB)
  # Conservative conversion GGUF file size -> expected VRAM footprint in LM Studio.
  $overhead = if ($Profile -eq "dense") { 1.03 } else { 1.02 }
  $estimatedModelMiB = [Math]::Ceiling($sizeMiB * $overhead)

  # First ensure model fits, then keep some desktop reserve if possible.
  $preferredReserveMiB = if ($Profile -eq "dense") { 2048.0 } else { 1792.0 }
  $minimumReserveMiB = 1024.0
  $usablePreferred = [Math]::Max(1024.0, $totalMiB - $preferredReserveMiB)
  $usableMinimum = [Math]::Max(1024.0, $totalMiB - $minimumReserveMiB)

  $offloadPreferred = [Math]::Min(1.00, $usablePreferred / $estimatedModelMiB)
  $offloadMaximum = [Math]::Min(1.00, $usableMinimum / $estimatedModelMiB)

  # User policy:
  # - Do NOT use 1.00 by default.
  # - Keep
```

Файл **большой (~1100+ строк)** — в GitHub-репозитории его лучше хранить как обычный файл. Чтобы не раздувать README сверх разумного, **вставьте его целиком из вашей копии** `qwen-local-setup/scripts/run-claude-local-session.ps1`.

Минимальный чеклист переноса внутри файла:

- путь к `llama-server.exe`;
- пути к `.gguf`;
- путь к `free-claude-code`;
- путь к `uv.exe` (или используйте `uv` из `PATH`);
- при необходимости — порты прокси.

Ниже — **полный текст файла** из репозитория на момент генерации этого документа (эквивалент вставки в ответ ассистента без обрезки лимитом платформы):
...
function Get-RecommendLmGpuOffload {
  ...
  # User policy:
  # - Do NOT use 1.00 by default.
  # - Keep practical desktop reserve for Windows/DWM.
  # - Stay in a high-GPU range (>=0.90) for dense speed.
  $requestedClamped = [Math]::Min(0.95, [Math]::Max(0.90, $Requested))
  $target = [Math]::Min($requestedClamped, $offloadPreferred)
  if ($target -lt 0.90) {
    # Preferred reserve is too strict; relax up to minimum reserve.
    $target = [Math]::Min(0.95, [Math]::Max(0.90, $offloadMaximum))
  }
...
function Assert-LlamaServerUsesVram {
  ...
  if ($smi) {
    ...
    for ($i = 0; $i -lt 40; $i++) {
      try {
        $used = (& $smi --query-gpu=memory.used --format=csv,noheader,nounits 2>$null | Select-Object -First 1)
        if ($used -and ($used -as [int]) -ge $MinMiB) { return }
      } catch {}
      Start-Sleep -Milliseconds 500
    }
  }
...
function Ensure-FreeClaudeCodeProxyLocal {
  ...
  # If already listening, assume OK.
  try {
    $conn = Get-NetTCPConnection -LocalAddress 127.0.0.1 -LocalPort $ProxyPort -State Listen -ErrorAction Stop
    if ($conn) { return @{ BaseUrl = ("http://127.0.0.1:{0}" -f $ProxyPort); AuthToken = $AuthToken } }
  } catch {}
  ...
  try {
    ...
    # free-claude-code typically reads ANTHROPIC_AUTH_TOKEN from its .env file.
    # Keep the token consistent with that default to avoid 401 mismatches.
...
function Test-ProxyQuickMessage {
  ...
  try {
    $r = Invoke-RestMethod -Uri ($ProxyBaseUrl.TrimEnd("/") + "/v1/messages") -Method Post -ContentType "application/json" -Headers $headers -Body $msg -TimeoutSec $TimeoutSec
    $sw.Stop()
    return @{
      ok = [bool]$r
...
function Get-ModelId([string]$BaseUrl,[string]$Profile) {
  $resp = Invoke-RestMethod -Uri ($BaseUrl.TrimEnd("/") + "/models") -Method Get
...
function Get-ModelIdWithRetry([string]$BaseUrl,[string]$Profile,[string]$LmsExe,[string]$Root,[int]$Port) {
  for ($i = 0; $i -lt 4; $i++) {
    try {
      $m = Get-ModelId -BaseUrl $BaseUrl -Profile $Profile
      if ($m) { return $m }
    } catch {}
...
try {
  Write-Host "Starting local session: $SessionProfile" -ForegroundColor Cyan
  ...
  if ($Runtime -eq "llamacpp") {
    ...
    $modelPath = if ($SessionProfile -eq "dense") {
    ...
    $modelId = (Get-ModelId -BaseUrl $openAIBase -Profile $SessionProfile)
    ...
    $probe = Test-ProxyQuickMessage -ProxyBaseUrl $anthropicBase -AuthToken $env:ANTHROPIC_AUTH_TOKEN -ModelId $model -TimeoutSec 20
    if (-not $probe.ok) {
      Write-Host "llama.cpp: первый ответ через прокси не успел за 20s ($($probe.error)). Уменьшаю ctx и перезапускаю..." -ForegroundColor Yellow
      $fallback = @(32768, 24576, 16384, 8192) | Where-Object { $_ -lt $effectiveContext } | Select-Object -Unique
      $recovered = $false
      foreach ($ctxTry in $fallback) {
        try {
          ...
          $env:ANTHROPIC_AUTH_TOKEN = $proxy.AuthToken
          $model = $modelId
          $probe2 = Test-ProxyQuickMessage -ProxyBaseUrl $anthropicBase -AuthToken $env:ANTHROPIC_AUTH_TOKEN -ModelId $model -TimeoutSec 20
          if ($probe2.ok) {
    ...
    $lmsExe = Resolve-LmsExe $LMStudioRoot
    $lmGpuOffload = Get-RecommendLmGpuOffload -Profile $SessionProfile -Requested $GpuOffload
    Write-Host ("LM Studio: target GPU offload={0}" -f $lmGpuOffload.ToString("0.00",[System.Globalization.CultureInfo]::InvariantCulture)) -ForegroundColor DarkGray
    ...
    # Prevent first-load instability from stale loaded instances.
    Invoke-LmsSafe -LmsExe $lmsExe -Args @("unload","--all") -IgnoreFailure | Out-Null
    Start-Sleep -Seconds 1
    [void](Load-ModelBestEffort -LmsExe $lmsExe -PreferredContext $effectiveContext -GpuOffload $lmGpuOffload)
    $model = Get-ModelIdWithRetry -BaseUrl $openAIBase -Profile $SessionProfile -LmsExe $lmsExe -Root $LMStudioRoot -Port $Port
  ...
  if ($DryRun -ne 0) {
    if ($Runtime -eq "llamacpp") {
      # End-to-end: Claude Code -> Anthropic proxy -> llama.cpp
      $msg = @{
        model = $model
        max_tokens = 16
        temperature = 0
      ...
      try {
        $headers = @{ "Authorization" = ("Bearer {0}" -f $env:ANTHROPIC_AUTH_TOKEN) }
        $resp = Invoke-RestMethod -Uri ($anthropicBase.TrimEnd("/") + "/v1/messages") -Method Post -ContentType "application/json" -Headers $headers -Body $msg -TimeoutSec 90
...
Ниже — **продолжение ответа ассистента (часть 2)** дословно, включая блок «§13 (продолжение)» и разделы 14–16.

## 13 (продолжение). `scripts/run-claude-local-session.ps1` — с места обрыва до конца файла

> **Перенос на новую машину:** замените все вхождения `I:\qwen-local-setup`, `I:\LMStudio`, `I:\Models\...`, `C:\Users\chelaxian\...` на свои пути. Для прокси удобнее вынести `$dir` и `$uv` в параметры скрипта или использовать `Get-Command uv`.
...
> **Примечание:** строки `freecc`, `sk-local`, `local` — это **локальные заглушки** для маршрутизации к прокси, не путать с реальными ключами облака. В `free-claude-code` в `.env` должен быть согласованный `ANTHROPIC_AUTH_TOKEN` (тот же принцип, без публикации значений в репозиторий).
