...
Данный проект позволяет автоматически разлогиниваться и залогиниваться на YouTube по заданному циклу. Скрипт реализован для расширений [Violentmonkey](https://violentmonkey.github.io/) (аналогично можно использовать Tampermonkey) в браузере Chrome.

## Необходимые расширения

1. **Violentmonkey**  
   [Ссылка на Chrome Web Store](https://chrome.google.com/webstore/detail/violentmonkey/jinjaccalgkegednnccohejagnlnfdag)  
   _Установите расширение для управления пользовательскими скриптами._

2. (Опционально) **LastPass** или другой менеджер паролей  
   _Для автозаполнения данных на странице логина Google, если требуется._
...
2. **Создание нового скрипта:**
   - Откройте панель Violentmonkey (иконка расширения).
   - Выберите "Создать новый скрипт" или "New script".
   - Удалите содержимое шаблона и вставьте финальный скрипт, приведённый ниже.

3. **Настройка скрипта:**
   - В начале скрипта можно отредактировать константу `INTERVAL` (интервал ожидания в миллисекундах, например, 300000 для 5 минут) и `WAIT` (ожидание между шагами, 3000 мс для 3 секунд).
...
1. **Шаг 0 (на главной странице YouTube):**
   ...
   - Устанавливает состояние цикла в "1".
...
3. **Шаг 2 (на странице логина):**
   ...
   - Сбрасывает состояние цикла в "0", и цикл повторяется.
...
3. Ждать 3 секунды.
4. Переходить на страницу логина Google (автозаполнение через LastPass, если настроено).
5. Ждать 3 секунды.
